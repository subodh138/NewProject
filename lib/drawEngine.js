...
function getLatestScoresForUser(db, userId) {
  return db
...
function hydrateEntryScores(db, entry) {
...
function settleDrawEntries(db, draw) {
  const winningNums = parseScores(draw.winning_numbers);
  const entries = db.prepare('SELECT * FROM draw_entries WHERE draw_id = ?').all(draw.id);

  const scoredEntries = entries.map((entry) => {
    const userNums = hydrateEntryScores(db, entry);
...
function recalculatePublishedDraw(db, drawId) {
