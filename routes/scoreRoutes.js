...
// POST /api/scores  — add a new score (auto-drops oldest if > 5)
router.post('/', authenticate, requireSubscription, [
  ...
  // Enforce rolling 5
  const existing = db.prepare('SELECT id FROM scores WHERE user_id = ? ORDER BY played_date DESC').all(userId);
  if (existing.length >= 5) {
    const oldest = existing[existing.length - 1];
    db.prepare('DELETE FROM scores WHERE id = ?').run(oldest.id);
  }

  const id = uuidv4();
  db.prepare('INSERT INTO scores (id, user_id, score, played_date) VALUES (?, ?, ?, ?)')
    .run(id, userId, score, playedDate);

  const all = db.prepare('SELECT * FROM scores WHERE user_id = ? ORDER BY played_date DESC').all(userId);
...
router.patch('/:id', authenticate, requireSubscription, [
  ...
  const db = getDb();
  const existing = db.prepare('SELECT id FROM scores WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
  if (!existing) return res.status(404).json({ error: 'Score not found.' });

  db.prepare('UPDATE scores SET score = ?, played_date = ? WHERE id = ? AND user_id = ?')
    .run(req.body.score, req.body.playedDate, req.params.id, req.user.id);

  const all = db.prepare('SELECT * FROM scores WHERE user_id = ? ORDER BY played_date DESC').all(req.user.id);
